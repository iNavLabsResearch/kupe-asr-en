"""Environment, HF auth, repo creation/push, wandb init, logging.

Secrets come from a .env file (auto-loaded) OR exported env vars:
    HF_TOKEN, HF_OWNER, WANDB_API_KEY, WANDB_PROJECT
...
log = logging.getLogger("kupe-asr-en")
...
def require_token() -> str:
    tok = hf_token()
    if not tok:
        raise RuntimeError("HF_TOKEN not set. Put it in .env or run: export HF_TOKEN=hf_xxx")
    return tok
...
def ensure_repo(repo_id: str, repo_type: str, private: bool = False) -> str:
    ...
    create_repo(repo_id, repo_type=repo_type, token=require_token(),
...
def upload_folder(folder: str, repo_id: str, repo_type: str, path_in_repo: str = "",
    ...
    _upload(folder_path=folder, repo_id=repo_id, repo_type=repo_type,
            path_in_repo=path_in_repo, token=require_token(),
            commit_message=commit_message, ignore_patterns=ignore_patterns)
...
def upload_file(local_path: str, repo_id: str, repo_type: str, path_in_repo: str,
    ...
    _upload(path_or_fileobj=local_path, path_in_repo=path_in_repo, repo_id=repo_id,
            repo_type=repo_type, token=require_token(), commit_message=commit_message)


def init_wandb(project_default: str, run_name: str, config: dict | None = None):
    """Init wandb if a key is present; else return None (training still runs)."""
    load_env()
